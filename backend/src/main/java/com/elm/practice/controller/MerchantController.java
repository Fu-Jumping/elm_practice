...
public class MerchantController {
    ...
    @GetMapping("/store") public ApiResponse<?> store(HttpSession s){var m=auth.requireMerchant(s);return ApiResponse.success(ViewMapper.merchantStore(stores.requireMerchantStore(m),m.phone));}
    @PatchMapping("/store") public ApiResponse<?> patchStore(@RequestBody Requests.StorePatch r,HttpSession s){var m=auth.requireMerchant(s);var st=stores.requireMerchantStore(m);stores.updateStore(m,st,r.name,r.description,r.image,r.startPrice,r.deliveryFee,r.contactPhone);return ApiResponse.success(ViewMapper.merchantStore(st,m.phone));}
    @PatchMapping("/store/status") public ApiResponse<?> status(@RequestBody Requests.StoreStatusPatch r,HttpSession s){var m=auth.requireMerchant(s);var st=stores.requireMerchantStore(m);try{stores.updateStatus(st,Domain.StoreStatus.valueOf(com.elm.practice.common.RequestUtil.required(r.status,"status")));}catch(IllegalArgumentException e){throw com.elm.practice.common.ApiException.badRequest("status 必须为 OPEN、CLOSED 或 TEMPORARILY_CLOSED");}return ApiResponse.success(ViewMapper.store(st));}
    ...
    @PatchMapping("/categories/{categoryId}/products") public ApiResponse<?> bindProducts(@PathVariable String categoryId,@RequestBody Requests.CategoryProductsPatch r,HttpSession s){catalog.bindProducts(auth.requireMerchant(s),categoryId,r.productIds);return ApiResponse.success(java.util.Map.of());}
    ...
    @PostMapping("/products") public ApiResponse<?> createProduct(@RequestBody Requests.ProductCreate r,HttpSession s){return ApiResponse.success(ViewMapper.product(catalog.createProduct(auth.requireMerchant(s),r)));}
    ...
    @PatchMapping("/products/{productId}") public ApiResponse<?> patchProduct(@PathVariable String productId,@RequestBody Requests.ProductPatch r,HttpSession s){return ApiResponse.success(ViewMapper.product(catalog.patchProduct(auth.requireMerchant(s),productId,r)));}
    ...
    @PutMapping("/products/{productId}/specifications") public ApiResponse<?> specifications(@PathVariable String productId,@RequestBody Requests.SpecificationsPatch r,HttpSession s){return ApiResponse.success(ViewMapper.product(catalog.saveSpecifications(auth.requireMerchant(s),productId,r.specOptions)));}
