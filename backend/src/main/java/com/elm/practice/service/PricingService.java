 ...
 * 金额一律 setScale(2, RoundingMode.HALF_UP)；超额优惠被截断到 0（TC-PRV-007），不返回负数。
 */
@Service
public class PricingService {
    ...
    public Result price(BigDecimal itemSubtotal, BigDecimal memberDiscountBase, BigDecimal deliveryFee,
        ...
        BigDecimal discountBase = memberDiscountBase == null ? ZERO : memberDiscountBase.setScale(2, RoundingMode.HALF_UP);
        var r = new Result();
        r.itemSubtotal = (itemSubtotal == null ? ZERO : itemSubtotal).setScale(2, RoundingMode.HALF_UP);
        r.packagingFee = PACKAGING_FEE.setScale(2, RoundingMode.HALF_UP);
        r.deliveryFee = (deliveryFee == null ? ZERO : deliveryFee).setScale(2, RoundingMode.HALF_UP);
        ...
        r.fullReductionAmount = full.setScale(2, RoundingMode.HALF_UP);
        ...
        r.newCustomerAmount = newUser.setScale(2, RoundingMode.HALF_UP);
        ...
        r.deliveryFeeDiscount = feeDiscount.setScale(2, RoundingMode.HALF_UP);
        ...
        r.memberDiscountAmount = member.setScale(2, RoundingMode.HALF_UP);
        // ⑥ 用户红包：批次⑥ coupons 表接入，当前恒 0。
        r.couponAmount = (couponAmount == null ? ZERO : couponAmount).setScale(2, RoundingMode.HALF_UP);
        ...
        r.total = total.setScale(2, RoundingMode.HALF_UP);
        return r;
    }
}
