...
- **唯一需求源**：`docs/PRD.md`。SRS 将 PRD 的用户可见行为转化为可实现、可测试的系统需求，不新增产品范围。
- **接口口径**：`docs/backend/后端接口契约.md`。本 SRS 只做模块映射和系统级约束，不复制接口字段明细。
...
P0 订单创建后状态为单一的 `PROCESSING`，支付、库存一致性、商家接单和状态推进属于已选 P1 扩展，除非后续验收安排明确，不作为本初稿的 P0 完成条件。
...
“联系商家”只表示订单用户与该订单商家之间的会话；消息、评价、优惠、经营统计等需在 P1 选定后再扩展。
...
测试编号沿用 `docs/project/TDD实施规划.md` 的矩阵编号；尚未创建的测试用例标记为“9/4 首批失败测试”。
...
| FR-PAY-01 | 模拟支付结果更新 | PRD 7.7 | 契约 3.5 `POST /orders/{id}/payment` | P1，未纳入 P0 |

追踪矩阵的完成判定是：需求有接口映射、接口有测试用例、测试通过后才可标记为完成。本文的测试编号只表示覆盖计划，不表示测试已经执行通过。
...
本地开发允许 Mock；正式联调必须使用 `VITE_API_MODE=real`、`VITE_API_BASE_URL=http://127.0.0.1:8080/api/v1`，并关闭 `VITE_MOCK_FALLBACK`。
...
6. P0 订单实付金额 = 商品小计 + 固定打包费 `2.00` 元；前端传入金额只用于提示或一致性检查。
...
字段类型和表结构以 `backend/` 建表脚本为最终工程实现；对外业务编号必须稳定，数据库内部主键不直接暴露为接口口径。
...
- 固定演示账号、店铺、商品和地址以接口契约第 2 章为准，不在 SRS 重复维护具体密码。
...
完整方法、路径、请求字段、响应字段和错误码以接口契约为准；后端实现发生调整时，必须同步回写契约与本 SRS 的映射，不允许只改前端。

### 6.3 接口一致性要求

1. 同一字段在用户端和商家端含义一致，状态值不能按页面自行改名。
2. 订单详情金额固定展示“商品合计、打包费、实付金额”三项。
3. 订单状态 P0 只使用 `PROCESSING`；P1 状态不得提前混入 P0 验收。
...
测试先运行得到失败结果，再实现最小行为；测试通过后再做重构和回归。失败原因必须是功能未实现或行为不符合约定，不能是测试自身错误。
...
以下事项不阻塞本 SRS 初稿提交，但必须在代码进入对应模块前确认并回写：

1. 后端 Spring Boot、MyBatis/JPA、MySQL 的精确版本。
2. `docs/backend/架构设计.md` 的最终模块划分、ER 图落点和事务实现方式。
3. 订单创建时库存是否在 P0 扣减，或仅在 P1 库存一致性扩展中实现；接口契约与 PRD 的最终决议优先。
4. SRS 第 2 章测试编号与测试联调同学实际文件名的最终对应。
