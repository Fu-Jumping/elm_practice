...
按 2026-09-09 第二阶段范围决议（PRD v1.1，并已按 v1.3 修订同步），**除平台管理员外的全部 P1 与 P2 已纳入本期**：模拟支付与库存一致性、商家订单推进与跨端同步、优惠与统一计价、评价与回复、消息、搜索排序与分页、收藏、会员、红包、规格、取消、图片上传（**配送已砍，CHG-002**）、经营统计与设置/FAQ；商家端原“二期不做”的 7 页与规格维护、联系顾客一并纳入。上述扩展当前均为“本期实现（待开发）”，未取得接口、数据库和测试证据前不得表述为已完成。
...
- 骑手定位、地图配送和实时物流跟踪（**简化配送管理与配送单已于 2026-09-11 砍掉，CHG-002**）。
...
| ~~配送单~~ | 简化配送管理的数据对象，状态为 PENDING_DELIVERY/DELIVERING/DELIVERED，由商家端推进，用户端只读（**2026-09-11 已砍掉，CHG-002**：配送状态改由订单状态表达，不建配送单） |
...
测试编号沿用 `docs/project/TDD实施规划.md` 的矩阵编号；尚未创建的测试用例标记为“9/4 首批失败测试”，扩展模块 TC 编号已在 TDD §5.20/§5.21 等小节登记，未登记的按「待补（本期实现（待开发））」处理。
...
| FR-MER-01 | 商家注册并创建店铺 | PRD 7.1 | 契约 4.1 `POST /merchants` | TC-MER-001~006 |
| FR-MER-02 | 商家登录和店铺信息维护 | PRD 7.1、7.13 | 契约 4.1 | TC-MER-007~012 |
| FR-STO-01 | 店铺列表、详情、分类和商品查询 | PRD 7.3 | 契约 3.2 `GET /stores/**`、`GET /products/{id}` | TC-STO-001~008 |
| FR-ADR-01 | 地址查询、新增、修改和删除 | PRD 7.9 | 契约 3.3 `GET/POST/PATCH/DELETE /me/addresses/**` | TC-ADR-001~009 |
...
| FR-CART-01 | 购物车查询、加购、改数量和删除 | PRD 7.3 | 契约 3.4 `/cart`、`/cart/items` | TC-CRT-001~010 |
| FR-ORD-01 | 从购物车创建基础订单 | PRD 7.4、7.6 | 契约 3.5 `POST /orders` | TC-ORD-001~012 |
...
| FR-PRM-01 | 优惠与后端统一计价 | PRD 7.4 | 契约 6.3 + §3.5 优惠计价接入下单（**已定稿**） | 待补（本期实现（待开发）） |
...
| FR-FAV-01 | 商家收藏 | PRD 7.10 | 契约 §3.7 商家收藏 | 待补（本期实现（待开发）） |
| FR-MBR-01 | 会员权益 | PRD 7.10 | 契约 §3.8 会员与红包 | 待补（本期实现（待开发）） |
| FR-RED-01 | 红包 | PRD 7.10、7.4 | 契约 §3.8 会员与红包 | 待补（本期实现（待开发）） |
| FR-RED-02 | 爆红包（CHG-001） | PRD 7.10、7.4 | 契约 §3.10 爆红包（购买套餐 / 爆红包）、§3.8（`couponId` 选用） | 待补（本期实现（待开发）） |
| FR-CXL-01 | 取消订单 | PRD 7.6 | 契约 §3.5 用户取消订单 | 待补（本期实现（待开发）） |
| FR-DLV-01 | 配送单与配送状态 | PRD 7.6 | —（已砍，契约 §10.2 同节亦已标注） | **已砍掉（CHG-002）** |
| FR-IMG-01 | 图片上传 | PRD 7.7、7.12 | 契约 §10.1 图片上传 | 待补（本期实现（待开发）） |
...
| FR-AI-01 | AI 点餐助手 | PRD 6.11.1 | 契约 §10.6 AI 点餐助手 | 后端已实现（2026-09-14）；前端页面待实现 |
...
| 后端 | JDK 17+、Spring Boot 3.5.5、MyBatis 3.0.4（2026-09-10 结案） | `127.0.0.1:4000` |
| 数据库 | MySQL 8（**已结案**） | `127.0.0.1:3306` |
...
扩展模块（优惠、红包、会员、规格、取消、评价、消息、图片、统计（配送已砍））的数据实体与字段由契约新增章节和数据库脚本确定，当前为“本期实现（待开发）”，落地后回填本节。
...
扩展模块（优惠与计价、评价与回复、消息、搜索分页、收藏、会员、红包、规格、取消、图片上传、统计（配送已砍））的接口按契约新增章节实施，当前为“本期实现（待开发）”；接口按契约 **§3.5–§3.10、§5、§10** 实施；未收口部分按「本期实现（待开发）」标注，映射见第 2 章。

### 6.3 接口一致性要求

1. 同一字段在用户端和商家端含义一致，状态值不能按页面自行改名。
2. 订单详情金额明细与用户端统一为「**基础四行 + 优惠项按实际发生展示**」（基础四行 / 优惠项按实际发生展示，2026-09-11 定稿，CHG-004）：基础四行为商品合计、打包费、配送费、实付金额；满减优惠、红包优惠等优惠项在金额非 0 时各占一行（**取代 2026-09-10 的固定四行表述**）。
...
| 扩展模块接口契约 | `docs/backend/后端接口契约.md` §3.5–§3.10、§5、§10（**已定稿**，未收口部分见契约 §9） | 待补（本期实现（待开发）） |
...
1. ~~后端 Spring Boot、MyBatis/JPA、MySQL 的精确版本。~~ → **已结案：Spring Boot 3.5.5 + MyBatis 3.0.4 + MySQL 8**（以 `backend/pom.xml` 与运行说明为准）。
...
6. 阶段 2 老师下发的需求变更内容尚未发布（2026-09-10 起按「待定」跟踪：负责人判断大概率不发布，不作为开发前置阻塞）：本组 2026-09-09 范围决议只落“本组范围决策”，不替代老师变更；变更到达后按“先补失败测试锁定新行为，再改业务代码并回归”执行。
