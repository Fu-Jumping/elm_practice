...
| # | 先决条件 | 现状 | 归口 |
| --- | --- | --- | --- |
| 1 | **完成一次重新发布** | 线上为 **`20260914-a2f15b1`（2026-09-14 发布）**，已含批次③④⑧（评价、消息、图片上传）；**9/15 的变更待再次发布** | `TODO-OPS-009`（林晨） |
| 2 | **nginx 补 `/uploads/` 代理** | **已补**：`deploy/nginx.conf:37-44` 已加 `location /uploads/ { proxy_pass http://127.0.0.1:4000/uploads/; }`（后端静态映射 `/uploads/**` 且只监听 `127.0.0.1:4000`，不加反代时浏览器请求会落到 `location /` 的 SPA 回退返回 HTML）。**待重新发布后复验**（发布前线上仍为旧配置） | 部署（林晨） |
| 3 | `client_max_body_size` | **已放宽到 4m**（`deploy/nginx.conf:37-44` 同批修改，原 2m 偏紧：multipart 还有 boundary/头部开销，恰好 2MB 的图片可能被 nginx 以 413 拒掉；应用侧 2MB 校验仍是权威）。**待重新发布后复验** | 部署（林晨） |
