...
日期：2026-09-14（成员B（用户端），与 `TODO-USER-107 ②` 同批产出）
...
| # | 先决条件 | 现状 | 归口 |
| --- | --- | --- | --- |
| 1 | **完成一次重新发布** | 线上为 **`20260914-a2f15b1`（2026-09-14 发布）**，已含批次③④⑧（评价、消息、图片上传）；**9/15 的变更待再次发布** | `TODO-OPS-009`（成员A（后端B）） |
| 2 | **nginx 补 `/uploads/` 代理** | **已补**：`deploy/nginx.conf:37-44` 已加 `location /uploads/ { proxy_pass http://127.0.0.1:4000/uploads/; }`（后端静态映射 `/uploads/**` 且只监听 `127.0.0.1:4000`，不加反代时浏览器请求会落到 `location /` 的 SPA 回退返回 HTML）。**待重新发布后复验**（发布前线上仍为旧配置） | 部署（成员A（后端B）） |
| 3 | `client_max_body_size` | **已放宽到 4m**（`deploy/nginx.conf:37-44` 同批修改，原 2m 偏紧：multipart 还有 boundary/头部开销，恰好 2MB 的图片可能被 nginx 以 413 拒掉；应用侧 2MB 校验仍是权威）。**待重新发布后复验** | 部署（成员A（后端B）） |
...
> 方式 A 下图片类验证依赖上面先决条件 2；**仓库侧配置已补齐（`deploy/nginx.conf:37-44`，含 4m 限流），但需重新发布后生效**；生效前用户端只能验证「上传成功、`url` 已写进提交」，无法验证「图片能显示」。若要先行确认后端映射本身是否生效，可在服务器上 `curl -I http://127.0.0.1:4000/uploads/<fileName>`（本机无法直连 4000）。
...
| 6 | 提交后打开返回的 `url` | 图片可正常显示 | 见先决条件 2（`deploy/nginx.conf:37-44` 已补 `/uploads/` 反代，**待重新发布后生效**）；另查服务器 `backend/uploads/` 是否存在且可写、`file.upload-dir` 配置 |
...
| 评价图片上传 | TC-IMG 组 | 未执行（先决条件 1 已满足：批次⑧ 随 `20260914-a2f15b1` 上线；先决条件 2/3 仓库侧已补齐（`deploy/nginx.conf:37-44`），**待重新发布后复验**，见 §0） | — | — |
...
- **本轮已满足的先决条件**：先决条件 1 的重新发布已完成（21:59–22:00，`/srv/elm-current → 20260914-a2f15b1`），线上后端已返回 `payDeadline`/`reviewed`/取消字段与真实会话摘要，批次②③⑦⑧ 接口已就位。（**2026-09-15 补注**：先决条件 2/3（`deploy/nginx.conf:37-44` 的 `/uploads/` 反代与 `client_max_body_size 4m`）仓库侧已补齐，但尚未重新发布，故本条记录当时仍不可做，现待重新发布后复验。）
