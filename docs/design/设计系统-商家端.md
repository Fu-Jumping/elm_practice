...
> 6. 侧边栏导航为**九项平铺（不分组）**：运营概览、订单、商品、分类、优惠配置、评价管理、消息、数据统计、店铺设置；**评价管理与评价回复合并为一页（列表 + 回复抽屉）**，规格维护在商品页内、聊天详情在消息页内，均不单独占菜单项（PRD 视觉标准摘要与 7.16.2；口径来源见 `docs/project/项目规则.md` §5）。
